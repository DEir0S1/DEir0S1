## Привет 👋

Меня зовут Калмыков Александр, я начинающий QA Engineer.
Занимаюсь освоением профессии а так же поиском первой работы.

🎯 За это время было освоено:
- Изучение Manual QA
- Написание тестовой документации (Чек-листы, тест кейсы, баг-репорты)
- Освоение API тестирования
- Работа с SQL запросами
- Изучение Git и GitHub

Примеры моих работ находятся в репозитории [QA Learning](https://github.com/DEir0S1/QA_learning)

В процессе осваиваю Java и фреймворки (Spring, Hibernate, JUnit) для дальнейшего выхода в автоматизацию. По мере освоения буду писать игру по мотивам [Rogue](https://github.com/DEir0S1/Rogue21)  

## 🚀 Планы по развитию
- Изучить Java и фреймворки
- Дописать Rogue, опробовать написание JUnit тестов
- Изучить автоматизацию тестирования
- Создать отдельный проект с Selenium/Playwright

### tg: [@DEir0S](https://t.me/DEir0S)


![QA](https://img.shields.io/badge/QA-Junior-blue)
![Postman](https://img.shields.io/badge/Postman-API-orange)
![SQL](https://img.shields.io/badge/SQL-Learning-green)
![Git](https://img.shields.io/badge/Git-Version%20Control-red)

---

⭐ Репозитории будут по возможности пополняться новыми материалами по тестированию и разработке.
